...
from ..settings import DATA_PATH, EVAL_PATH, ROOT_PATH# datapath is data/ .. 
...
import os
"""
python -m gluefactory.eval.treeEval1 --checkpoint <modelName> --overwrite
python -m gluefactory.eval.treeEval1 --conf <conf_file_name> --overwrite
"""

logger = logging.getLogger(__name__)

pairsPath = "syntheticForestData/pairs_info_calibrated_relative_3x3.txt"
...
class ForestPipeline(EvalPipeline):
    default_conf = {
        "data": {
            "name": "image_pairs",   
            "pairs": pairsPath, 
            # is e.g. SF_E_R_P001/filename.jpg  SF_E_R_P001/filename.jpg intrinsic1 intrinsic2  poses: tx ty tz qx qy qz qw
    ...
    def get_predictions(self, experiment_dir, model=None, overwrite=False):
        ...
        def backup_existing_file(file_path):
            if file_path.exists():
                ...
                print(f"Existing file {file_path} renamed up to {backup_path}")
    ...
    def run_eval(self, loader, pred_file, usingExtraStats=True):
        ...
        conf = self.conf.eval
        onlyTiming = False
        print("run eval the loader is", loader)
        ...
        for i, data in enumerate(tqdm(loader)):
            ...
            for k, v in results_i.items():
                results[k].append(v)
        summaries = {}
        # summaries results as a dict[str, float]
        # you can also add your custom evaluations here
        summaries["z-time"] = self.timeData["avg_inference_time"]
        summaries["z-num_samples"] = self.timeData["num_samples"]
        if not onlyTiming:
...
if __name__ == "__main__":
    ...
    updateFile = True
    usingPretrainedModels = True
    usingOnlyTiming = False
    ...
    pprint(s)
    if not updateFile:
        exit("exiting form treeEval1pt to avoid saving")

    print("before checkpoint used is ", args.checkpoint)
    print("args conf used is ", args.conf)
    ...
    filePath = f"{str(DATA_PATH)}/{txtfolder}/{args.checkpoint}/{dataset_name}_{estimator_type}.txt" 
    ...
    if usingExtraStats:
        args.plot = True
        savePath = str(ROOT_PATH)
        if args.plot:
            # count = 0
            for name, fig in f.items():
                fig.canvas.manager.set_window_title(name)
                # finalPath = savePath +  "/evalFigures/" + name + {dataset_name}_{args.checkpoint}_{estimator_type} + str(count) + ".png"
